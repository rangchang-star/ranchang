...
export default function AICognitionPage() {
  ...
  useEffect(() => {
    async function loadAssessment() {
      setIsLoading(true);
      try {
        const res = await fetch(`/api/assessments/ai-cognition`);
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch("/api/assessments/ai-cognition/submit", {
        method: "POST",
      ...
      if (data.success) {
        // 跳转到结果页面
        window.location.href = `/assessment/ai-cognition/result?id=${data.resultId}`;
      }
    } catch (error) {
