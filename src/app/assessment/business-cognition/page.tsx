...
export default function BusinessCognitionPage() {
  ...
  useEffect(() => {
    async function loadAssessment() {
      setIsLoading(true);
      try {
        const res = await fetch(`/api/assessments/business-cognition`);
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch("/api/assessments/business-cognition/submit", {
        method: "POST",
      ...
      const data = await res.json();
      if (data.success) {
        window.location.href = `/assessment/business-cognition/result?id=${data.resultId}`;
      }
    } catch (error) {
