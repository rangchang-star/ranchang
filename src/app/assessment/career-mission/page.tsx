...
export default function CareerMissionPage() {
  ...
  useEffect(() => {
    async function loadAssessment() {
      setIsLoading(true);
      try {
        const res = await fetch(`/api/assessments/career-mission`);
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch("/api/assessments/career-mission/submit", {
        method: "POST",
      ...
      const data = await res.json();
      if (data.success) {
        window.location.href = `/assessment/career-mission/result?id=${data.resultId}`;
      }
    } catch (error) {
